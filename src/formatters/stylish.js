...
const objToString = (obj) => Object.entries(obj).map(([key, value]) => `${key}: ${value}`);

const stringify = (value, depth) => (value instanceof Object
...
const typeActions = {
  nested: ({ name, value }, depth, fn) => `  ${indent(depth)}${name}: ${fn(value, depth + 2)}`,
